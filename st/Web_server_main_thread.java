...
public class Web_server_main_thread extends javax.swing.JFrame {
     ...
     * Server name
     */
    public final static String SERVER_NAME = "Demo HTTP Serv 2023/2024";
    /**
     * Default html directory TASK 6.0 - change this constant to have the
     * location of the html directory in your computer
     */
    public final static String HTML_DEFAULT_DIRECTORY = "../html";
    /**
    ...
    /**
     * ******************************
     * TASK 7.1: define and create a new HashMap object to store
     * User_credentials objects.
     */
    // private final HashMap<String, User_credentials> user_database = ...
    private final HashMap<String, User_credentials> user_database = new HashMap<>();
    /**
     * Constructor
     */
    ...
    private void initComponents() {
        ...
        jButtonTest.setForeground(new java.awt.Color(0, 0, 0));
    ...
    private void jToggleButtonActiveActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jToggleButtonActiveActionPerformed
        if (jToggleButtonActive.isSelected()) {
            ...
            /**
             * ******************************
             * TASK 5.1: create a new DatagramSocket ds and configure it.
             */
            // Remember to add try / catch as is described in the tutorial document:
            try {
                // ################## 5.1
                ds = new DatagramSocket(tcp_port);
            ...
            try {
                /**
                 * ******************************
                 * TASK 5.1: close the DatagramSocket ds and free the object.
                 */
                // ...
    ...
    boolean send_AUTH_REQ(InetAddress servIP, int servPort, String user, int seq) {
        ...
        try {
            /**
             * ******************************
             * TASK 5.2: prepare and send the AUTH_REQ message. The structure is
             * the following:
             */
            ...
            /**
             * ******************************
             * Start by writing the various elements to "dos" copy to a byte
             * array construct a datagram object See UDP chat for enlightening
             */

            dos.writeByte(AUTH_REQ);
            dos.writeInt(seq);
            ...
            dp.setAddress(servIP);
            dp.setPort(servPort);
            ds.send(dp);

            ds.setSoTimeout(1000); // ################ 5.4

            Log("Sent AUTH_REQ (" + seq + ";" + user + ") to " + servIP + ":" + servPort + "\n");
            return true;
        } catch (SocketTimeoutException e) {
            Log("No packet within 1 seconds");
            return false;
        } catch (Exception e) {
    ...
    private User_credentials get_challengeresponse_from_server(String user) {
        ...
        try {
            ...
            /**
             * ******************************
             * TASK 5.3: a) Read all the fields of AUTH_REP, using the dis
             * object, according to the protocol to the local variables declared
             * above b) Test if the values received match the ones sent (user,
             * seq) c) Create a new User_credentials object with the data
            ...
            //ruser = dis.readAllBytes();

            byte[] sbuf = new byte[len_msg];
            int n = dis.read(sbuf, 0, len_msg); // reads bytes and returns number of bytes read
            ruser = new String(sbuf, 0, n);

            rseq = dis.readInt();
            dis.readShort();
            sbuf = new byte[3];
            n = dis.read(sbuf, 0, 3);
            response = new String(sbuf, 0, n);
            ...
            */
            ...
            if (challenge.length == 0 || response.length() <= 0) {
                Log("Received AUTH_REP (no user) from " + rdp.getAddress().getHostAddress() + ":" + rdp.getPort() + "\n");
                return null;
            }

            // Implement b) - Validate rseq and ruser
            if (!ruser.equals(user) || seq != rseq) {
                Log("User '" + ruser + "' doesn't exist\n");
                return null;
            }
            ...
            User_credentials re = new User_credentials(user, challenge, response, get_Lifetime_value());

            /**
             * ******************************
             * TASK 7.2 Store the User_credentials object in the database
             * created in task 7.1 when a valid challenge-response vector is
             * received.
             */
            // ...
            user_database.put(response, re);
            return re;
        } catch (SocketTimeoutException e) {
            Log("No packet within 1 second\n");
            return null;
    ...
    private User_credentials get_cached_challengeresponse(String user) {
        ...
        /**
         * ******************************
         * TASK 7.3 Lookup the user_database for the user, and return a cached
         * User_credentials if it is still valid.
         */
        // ...

        
        for(User_credentials u : user_database.values()){
            if(u != null){
                if(u.get_user().equals(user)){
                    if(u.is_valid()){
                        return u;
                    }
                }
    ...
    /**
     * Function that gets a User_credentials. First sees if it is in cache and
     * gets a new one if none is available
     *
     * @param user user id
