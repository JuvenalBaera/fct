...
public class Chat_tcp extends javax.swing.JFrame {
    ...
    private void initComponents() {
        ...
        jFileChooserSendFile = new javax.swing.JFileChooser();
    ...
    private void jToggleButtonConnectActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jToggleButtonConnectActionPerformed
        if (jToggleButtonConnect.isSelected()) {
            ...
            if (local_connection != null) {
                ...
                connlist.remove(local_connection.toString());
    ...
    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
        ...
        File f;
        if (jFileChooserSendFile.showSaveDialog(this) == jFileChooserSendFile.APPROVE_OPTION) {
            f = jFileChooserSendFile.getSelectedFile();
            if (connlist.isEmpty()) {
                Log_loc("There is no connection\n");
                return;
            }
            for (Connection_tcp c : connlist.values()) {
                if (c.send_file(f)) {
                    //Log_loc("File sended  : Size: " + f.length() + "\nName: " + f.getName() + "\n");
                    //Log_rem("File received: Size: " + f.length() + "\nName: " + f.getName() + "\n");
                    Log_loc(formatter.format(new Date()) + " - sent file to "
                            + c.toString() + " - '" + f.getName() + "'\n");
                }
            }
        }
    ...
    public void connection_thread_ended(Connection_tcp th) {
        ...
        if(th == local_connection){
            set_local_connection(null);
        }
        connlist.remove(th.toString());
    ...
    private javax.swing.JFileChooser jFileChooserSendFile;
