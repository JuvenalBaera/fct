...
public class Terminal extends javax.swing.JFrame implements Log, Simulator {
    ...
    private static final int DATA_TIMER_ID = 3;
    ...
    private void jComboBoxProtocolActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jComboBoxProtocolActionPerformed
        ...
        switch (n) {
            ...
            case 5:
                ...
                break;
            case 6:
                jTextSndWND.setText("" + (1+maxs)/2);
                jTextRcvWND.setText("" + (1+maxs)/2);
                break;
        }
        ...
        if (ev.str_to_event(message, this)) {
            ...
            switch (ev.kind()) {
                ...
                case Event.FRAME_EVENT:
                    ...
                    // Test ackvec
                    if (ev.frame().ackvec() != null) {
                        if (!ev.frame().valid_ackvec(this.get_max_sequence())) {
                            Log(time + " Invalid ackvec contents!\n");
                        }
                    }
    ...
    public void start_data_timer() {
        try {
            ...
            Event ev = Event.new_Start_Timer(this, time, DATA_TIMER_ID, delay);
            Log(time + " start_data_timer (" + delay + ")\n");
            data_timer_on.put(DATA_TIMER_ID, time);
            String txt = ev.event_to_str();
            conn.send_message(txt);
        } catch (Exception ex) {
    ...
    public void cancel_data_timer() {
        try {
            Event ev = Event.new_Start_Timer(this, time, DATA_TIMER_ID, -1);
            if (data_timer_on.containsKey(DATA_TIMER_ID)) {
                data_timer_on.remove(DATA_TIMER_ID);
    ...
    /**
     * Test if data timer is active
     */
    @Override
    public boolean isactive_data_timer() {
        return data_timer_on.containsKey(DATA_TIMER_ID);
    }
