...
public class Thread_tcp_connection extends Thread {
    ...
    public String make_login1Page(String ip, int port, String user, String message) {
        ...
        html += "<h1 align=\"center\"><font color=\"#800000\">Sistemas de Telecomunica&ccedil;&otilde;es</font> <font color=\"#c0c0c0\">2023/2024</font></h1>\r\n";
    ...
    private void return_file_content_to_browser(String filename, String set_cookie_value) {
        try {
            ...
            if (set_cookie_value != null) {
                /**
                 * ******************************
                 * TASK 6.3. Send a Set-Cookie header field to the browser with
                 * the content of the string set_cookie_value, if this is not
                 * null.
                 */
                // ...
                print_out.print("Set-Cookie: " + set_cookie_value
                        + "; expires=Friday, 25-Apr-25 23:12:40 GMT; path="
                        + filename + "\r\n");
    ...
    private void return_string_content_to_browser(String html) {
        ...
        /**
         * ******************************
         * TASK 6.1: Send a) the HTTP reply headers b) the line \r\n c) the
         * content of html variable to the browser.
         */

        print_out.print("HTTP/1.0 200 OK\r\n");
        print_out.print("Content-Type: text/html; charset=UTF-8\r\n");
        print_out.print("Content-Length: " + html.length() + "\r\n");
        print_out.print("\r\n");

        print_out.print(html);
        // main_thread.Log("return_string_content_to_browser not implemented yet\n");
    }
     ...
     * If it is a GET it might appear the "Cookie", if it is a POST it appears
     * the "Content-Length" and the answers. Returns the object with the data
     * read.
     *
    ...
    public boolean valid_KEY(String key) {
         ...
         * Form 2 or a cookie. Suggestions: You need the key to get the
         * username, and the username to get a User_credentials object in cache
         * or from the authentication server. Finally, use User_credentials's
         * method validate_client_string to check if the key is a valid reply to
         * the challenge.
         */
        ...
        User_credentials user_cred = this.main_thread.get_challengeresponse(user);

        if (user_cred == null) {
            return false;
        }
        if (!user_cred.validate_client_string(key)) {
            return false;
        }
        return true;
    }
    ...
    public void run() {
        try {
             ...
             * following actions: a) Try to read the HTTP headers (Cookie,
             * Content-Length, answers) b) If it is a GET with a cookie, or a
             * POST with answers, get the username and validate it using the
             * get_challengeresponse method c) If the user is valid, send the
             * page requested, or "/", to the browser d) If the user is not
             * valid, or was not sent, send the Form 1 to the browser
             */
            /**
             * ******************************
             * TASK 6.2 a) Try to read the HTTP headers (Cookie, Content-Length,
             * answers) Suggestion: Use the function
             * read_cookie_from_GET_or_answer_from_POST(...) to read the fields
             * and return a GET_or_POST_answers object
             */
            /**
             * ******************************
             * TASK 6.2 b) Test if it is a GET with a cookie, or a POST. Get the
             * username and validate it using the get_challengeresponse method
             * Suggestions: Use the function get_user_from_cookie(...) to get
             * the user name from the cookie or from the POST answer. If the
             * user is not valid use function make_login1Page to create form 1
             * and function return_string_content_to_browser to send form 1 to
             * the browser.
             */
            String user = null;
            String key = null;
            GET_or_POST_answers answers = null;
            User_credentials userCred = null;

            if (method.equals("GET")) {
                answers = read_cookie_from_GET_or_answer_from_POST(false);

                if (answers.get_cookie() != null) { // Has cookie
                    main_thread.Log("\nGET - cookie\n");
                    user = get_user_from_cookie_or_answer(answers.get_cookie());
                    userCred = this.main_thread.get_challengeresponse(user);
                    if (userCred != null) {
                        return_string_content_to_browser(file);
                    } else {
                        return_error_to_browser();
                    }
                } else { // No cookie
                    //main_thread.Log("\nGET - No cookie\n");
                    String f1 = make_login1Page(this.main_thread.ssock.getInetAddress().toString(),
                            this.main_thread.ssock.getLocalPort(), "user", "Enter Username");
                    return_string_content_to_browser(f1);
                }
            } else { // POST
                answers = read_cookie_from_GET_or_answer_from_POST(true);
                if (answers != null) {
                    key = answers.get_POST_answers();

                    if (valid_KEY(key)) { // Second 
                        main_thread.Log("Valid Keyyyyyy\n");
                    } else {
                        user = get_user_from_cookie_or_answer(key);
                        userCred = this.main_thread.get_challengeresponse(user);

                        if (userCred != null) {

                            //if(userCred.has_challenge()){
                            String f1 = make_login2Page(
                                    soc_to_browser.getInetAddress().getHostAddress(),
                                    soc_to_browser.getPort(),
                                    user,
                                    userCred.get_challenge(),
                                    "Enter the characters of your password");
                            return_string_content_to_browser(f1);
                        } else {
                            return_error_to_browser();
                        }
                ...
                /**
                 * ******************************
                 * TASK 7.4. If a valid user is received after Form 1 and a
                 * challenge-response pair is received from the authentication
                 * server, the web server should send the Form 2 to the browser
                 * with the challenge received. Suggestions: Use function
                 * make_login2Page to create form 2 and function
                 * return_string_content_to_browser to send form 2 to the
                 * browser.
                 */
                ...
                /**
                 * ******************************
                 * TASK 7.6. Modify your code to only send the web page contents
                 * when a valid key is received in a POST to form 2 or in a
                 * cookie.
                 */
                // Return the file contents to the browser
                /**
                 * ******************************
                 * TASK 6.4. Add a Set-Cookie string when a file is transmitted
                 * to the browser
                 */
                //return_file_content_to_browser(file, null);
                return_file_content_to_browser(file, "user=" + user);
                //return_file_content_to_browser("/", "user="+user);
            }
            }catch (IOException e) {
            System.out.println("I/O error " + e);
        }catch (Exception e) {
